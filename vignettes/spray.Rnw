...
\noindent But again see how the R object thus created stores zero
elements, which can be problematic if the polynomial in question is
large degree and sparse.  Similar issues arise in the case of
multivariate polynomials.  The~\pkg{multipol}
...
anonymous R Journal referee for this insight.}.
...
in the cell); and work on a~$17\times 17$ lattice.
