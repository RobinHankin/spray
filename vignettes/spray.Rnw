...
This is realized in the package using the \code{polyform} option,
...
\section{Manipulation of coefficients}

The \pkg{spray} package uses \pkg{disordR}
discipline~\citep{hankin2022} for manipulation of the coefficients.
Thus:
